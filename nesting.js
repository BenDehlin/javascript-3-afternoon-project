...
function removeDuplicates(){
  for(i = 0; i < workplaceAccidents.length; i++){
    for(i2 = 0; i2 < workplaceAccidents.length; i2++){
      if(workplaceAccidents[i] === workplaceAccidents[i2] && i !== i2){
        workplaceAccidents.splice(i2, 1)
      }
    }
  }
...
function looper(){
  for(let i = 0; i < numsArr.length; i++){
    for(let i2 = 0; i2 < numsArr[i].length; i2++){
      if(numsArr[i][i2] % 2 === 0){
        numsArr[i][i2] = 'even'
      }else if (numsArr[i][i2] % 2 !== 0){
        numsArr[i][i2] = 'odd'
      }
    }
  }
